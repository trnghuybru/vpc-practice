        ...
        with c.cursor() as cur:
            ...
            if cur.fetchone()["n"] == 0:
                ...
                """, [
                    ...
                    ("ALB viết tắt của?", "Amazon Linux Build", "Application Load Balancer",
                     "Auto Link Balancer", "App List Base", "B"),
                    ("Lệnh tạo venv Python?", "python -m venv venv","pip install venv",
                     "virtualenv install","pyenv create","A"),
                ])

# --- Lazy init thay cho before_first_request (Flask 3 không còn) ---
_initialized = False
def ensure_init():
    global _initialized
    if not _initialized:
        try:
            ...
            # không raise ở đây để /health vẫn 200; các route DB sẽ báo lỗi riêng
    ...
    try:
        with get_conn(DB_NAME) as c:
            with c.cursor() as cur:
                ...
                key = {str(r["id"]): r["correct_option"] for r in cur.fetchall()}
...
def on_500(err):
    return (f"Lỗi hệ thống: {getattr(err, 'description', 'Internal error')}", 500)
